...
| DES-18 | Workspace mobile responsive layout — collapse `.ws-sidebar` (TOC) into a left off-canvas drawer on viewports below ~900px, add a hamburger toggle in a small mobile header, backdrop overlay, lock body scroll when open | P1 | Scoped to the LEFT sidebar / TOC only. The right investigation column is handled by DES-19's bottom-bar pattern. Replaces the umbrella DES-07 for the workspace. Touches `app/case/[id]/page.tsx`, `components/workspace/CaseSidebar.tsx`, `styles/globals.css`. Right-click annotations need a long-press equivalent or fallback affordance on touch (track separately if needed). |
| DES-19 | Progressive-disclosure investigation panel — by default collapse `.invest-column` to an icon-only rail (one icon per tab: Hypotheses, Resolution, OSINT, Links, Search, Score, Branches, Discuss); clicking an icon expands the full panel for that tab over the rail; clicking the active icon (or an X) collapses it back. On mobile, render the same icons as a fixed bottom bar with the drawer sliding up vertically instead of expanding sideways. Goal: avoid bombarding the user with eight tabs of analysis on first load — let them choose what to dig into. | P1 | Touches `components/workspace/InvestColumn.tsx` (state for collapsed/expanded + active tab, icon-only render mode), `app/case/[id]/page.tsx` (layout adapts to collapsed rail width), `styles/globals.css` (transition animations, mobile bottom-bar variant + vertical drawer), needs an icon set (Lucide already in deps if available; otherwise inline SVGs). Default state: collapsed. Persist last-opened tab in localStorage so power users don't re-open it every visit. Tooltip on hover for each icon (desktop). On mobile this entirely replaces the side drawer pattern for the right column. |
